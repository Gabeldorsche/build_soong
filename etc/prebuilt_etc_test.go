...
func TestPrebuiltGpuDirPath(t *testing.T) {
	targetPath := "out/soong/target/product/test_device"
	tests := []struct {
		description  string
		config       string
		...
		description: "prebuilt: system gpu",
		config: `
			prebuilt_gpu {
				name: "foo.conf",
				src: "foo.conf",
			}`,
		expectedPath: filepath.Join(targetPath, "system/gpu"),
	}, {
		description: "prebuilt: vendor gpu",
		config: `
			prebuilt_radio {
				name: "foo.conf",
				src: "foo.conf",
		...
		expectedPath: filepath.Join(targetPath, "vendor/gpu/sub_dir"),
	}}
	for _, tt := range tests {
		t.Run(tt.description, func(t *testing.T) {
			result := prepareForPrebuiltEtcTest.RunTestWithBp(t, tt.config)
